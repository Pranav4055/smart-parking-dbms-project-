    ...
    <div className="min-h-screen bg-white flex">
      <div className="w-full lg:w-1/2 flex items-center justify-center p-8">
        <div className="w-full max-w-md">
          <h1 className="text-4xl sm:text-5xl tracking-tighter font-black text-zinc-950 mb-2" style={{ fontFamily: 'IBM Plex Sans, sans-serif' }}>Get Started</h1>
          <p className="text-base text-zinc-600 mb-8">Create your parking account</p>

          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="text-xs tracking-[0.2em] uppercase font-mono text-zinc-500 block mb-2">Full Name</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-4 py-3 border border-zinc-200 bg-white text-zinc-950 focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 rounded-none" placeholder="John Doe" required />
            </div>
            <div>
            ...
            <div>
              ...
              <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3 border border-zinc-200 bg-white text-zinc-950 focus:outline-none focus:ring-2 focus:ring-zinc-900 focus:ring-offset-2 rounded-none" placeholder="••••••••" required minLength={6} />
            </div>
          ...
          <p className="text-sm text-zinc-600 mt-6 text-center">
            Already have an account? <Link to="/login" className="text-zinc-900 font-semibold hover:underline">Sign in</Link>
          </p>

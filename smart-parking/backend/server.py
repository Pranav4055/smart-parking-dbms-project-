...
# Security defaults. Set COOKIE_SECURE=false only for local HTTP development.
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        response.headers.setdefault("Content-Security-Policy", "default-src 'self'; script-src 'self' https://checkout.razorpay.com; connect-src 'self' https://api.razorpay.com; frame-src https://api.razorpay.com https://checkout.razorpay.com; img-src 'self' data: https://images.unsplash.com; style-src 'self' 'unsafe-inline'; font-src 'self' data:")
...
def enforce_rate_limit(request: Request, scope: str, limit: int = _RATE_LIMIT_MAX) -> None:
    now = time.monotonic()
...
class ParkingConfigCreate(BaseModel):
    ...
    slots_total: int = Field(ge=1, le=100000)
    price_per_hour: float = Field(ge=0, le=1000000)
...
class BookingCreate(BaseModel):
    vehicle_number: str = Field(min_length=2, max_length=20, pattern=r"^[A-Za-z0-9 -]+$")
    vehicle_type: str = Field(min_length=1, max_length=40)
    duration_hours: float = Field(gt=0, le=24)
    booking_type: Literal["online", "on-spot"]

class BookingExtend(BaseModel):
    additional_hours: float = Field(gt=0, le=24)
...
class SettingsUpdate(BaseModel):
    cancellation_type: Literal["fixed", "percentage"]
    cancellation_value: float = Field(ge=0, le=1000000)
...
async def update_parking_config(
    ...
    if config.vehicle_type != vehicle_type:
        raise HTTPException(status_code=400, detail="Vehicle type cannot be changed through this endpoint")
...
async def create_booking(
    ...
    slot_number = await assign_next_slot(booking.vehicle_type)
    amount = round(config["price_per_hour"] * booking.duration_hours, 2)
...
async def extend_booking(
    ...
    booking_oid = parse_object_id(booking_id)
...
async def startup_event():
    ...
    await db.parking_config.create_index("vehicle_type", unique=True)
    await db.bookings.create_index([("user_id", 1), ("created_at", -1)])
    await db.bookings.create_index([("vehicle_type", 1), ("status", 1)])
    ...
    if admin_email and admin_password:
        ...
        logger.warning("ADMIN_EMAIL/ADMIN_PASSWORD are not configured; skipping admin seeding")
